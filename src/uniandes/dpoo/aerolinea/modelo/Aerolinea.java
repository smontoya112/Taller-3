...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	if (contador==0) {
    		throw new Exception("No existe el avion");
    	}
    	if(!rutas.containsKey(codigoRuta)) {
    		throw new Exception("No existe la ruta");
    	}
    	for(Vuelo vuelo:vuelos) {
    		if(vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha) && vuelo.getRuta().getCodigoRuta().equals(codigoRuta)) {
    			throw new Exception("El vuelo ya existe");
    		}
    		
    		if(vuelo.getAvion().getNombre().equals(nombreAvion) && vuelo.getFecha().equals(fecha) ) {
    			throw new Exception("El avion esta ocupado");
    		}
    	
    	Vuelo vueloNew = new Vuelo(rutas.get(codigoRuta),fecha,avionSi);
    	vuelos.add(vueloNew);
    	}
    ...
    {
    	int tarifa = 0;
    	int mes = fecha.charAt(5)*10 + fecha.charAt(6);
